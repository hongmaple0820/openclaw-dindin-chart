...
router.post('/messages', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: (error as Error).message, code: 'INTERNAL_ERROR' });
  }
});
...
router.put('/bots/:id', async (req, res) => {
  try {
    ...
    if ((error as Error).message.includes('不存在')) {
      return res.status(404).json({ success: false, error: (error as Error).message, code: 'NOT_FOUND' });
    }
    res.status(500).json({ success: false, error: (error as Error).message, code: 'INTERNAL_ERROR' });
  }
});
