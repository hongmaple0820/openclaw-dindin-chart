...
router.post('/init', adminMiddleware, async (req, res) => {
  try {
    ...
    console.error('[Wecom] 初始化失败:', (error as Error).message);
    res.status(500).json({
      success: false,
...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('[Wecom] 发送失败:', (error as Error).message);
    res.status(500).json({
      success: false,
      error: (error as Error).message,
      code: 'SEND_ERROR'
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    console.error('[Wecom] 回调处理失败:', (error as Error).message);
...
router.get('/webhook', async (req, res) => {
  try {
    ...
    console.error('[Wecom] 回调验证失败:', (error as Error).message);
    res.status(500).send('error');
  }
});
