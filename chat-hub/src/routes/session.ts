...
router.post('/:sessionId/subscribe', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '订阅失败', error: (error as Error).message });
  }
});
...
router.post('/:sessionId/unsubscribe', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '取消订阅失败', error: (error as Error).message });
  }
});
