...
router.post('/:id/invite', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '邀请失败', error: (error as Error).message });
  }
});
...
router.delete('/:id/members/:targetUserId', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '移除失败', error: (error as Error).message });
  }
});
...
router.put('/:id/transfer', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '转让失败', error: (error as Error).message });
  }
});
...
router.delete('/:id', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '解散失败', error: (error as Error).message });
  }
});
...
router.put('/:id/members/:targetUserId/nickname', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '设置失败', error: (error as Error).message });
  }
});
...
router.post('/:id/leave', authMiddleware, (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '退出失败', error: (error as Error).message });
  }
});
