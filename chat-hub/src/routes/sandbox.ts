...
async function deleteSandbox(req, res) {
  try {
    ...
    if ((error as Error).message === '沙箱不存在') {
...
async function executeCode(req, res) {
  try {
    ...
    if ((error as Error).message === '沙箱不存在' || (error as Error).message === '沙箱未运行') {
  ...
  try {
    ...
    if (sandbox.status === 'running' && sandbox.containerId) {
      try {
        ...
        console.warn('[Sandbox] Failed to get container stats:', (e as Error).message);
      }
    }
