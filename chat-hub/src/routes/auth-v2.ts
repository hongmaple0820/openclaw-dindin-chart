...
router.post('/register', async (req, res) => {
  try {
    ...
    console.error('[Auth] 注册失败:', (error as Error).message);
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.error('[Auth] 登录失败:', (error as Error).message);
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    console.error('[Auth] 刷新令牌失败:', (error as Error).message);
    res.status(500).json({ success: false, error: (error as Error).message });
  }
});
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    console.error('[Auth] 发送验证码失败:', (error as Error).message);
    res.status(500).json({ success: false, error: (error as Error).message });
  }
});
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    console.error('[Auth] 重置密码失败:', (error as Error).message);
    res.status(500).json({ success: false, error: (error as Error).message });
  }
});
...
router.post('/change-password', authenticate, async (req, res) => {
  try {
    ...
    console.error('[Auth] 修改密码失败:', (error as Error).message);
    res.status(500).json({ success: false, error: (error as Error).message });
  }
});
