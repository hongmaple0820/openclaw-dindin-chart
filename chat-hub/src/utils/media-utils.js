...
class MediaUtils {
  ...
  async getImageDimensions(filePath) {
    try {
      ...
      // 如果sharp不可用，尝试其他方法
      console.warn('Sharp not available, skipping dimension detection:', error.message);
  ...
  async getMediaDuration(filePath) {
    try {
      ...
      console.warn('FFmpeg not available or error getting duration:', error.message);
      return null;
    }
  }
   ...
   * 创建图像缩略图
   */
  async createThumbnail(filePath, outputFile, maxWidth = 200, maxHeight = 200) {
    try {
      const sharp = (await import('sharp')).default;
      ...
      console.warn('Error creating thumbnail:', error.message);
      return null;
    }
  }
   ...
   * 验证文件类型
   */
  validateFileType(buffer, filename, allowedTypes = []) {
   ...
   * 保存媒体文件
   */
  async saveMediaFile(fileBuffer, originalName) {
    try {
...
if (require.main === module) {
  console.log('MediaUtils module loaded successfully');
}