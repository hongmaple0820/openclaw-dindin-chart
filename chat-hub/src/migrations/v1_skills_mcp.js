...
async function runMigration(db) {
  ...
  try {
    ...
    // 返回统计
    const stats = {
      builtin_skills: await countTable(db, 'builtin_skills'),
      marketplace_skills: await countTable(db, 'marketplace_skills'),
      user_skills: await countTable(db, 'user_skills'),
      custom_skills: await countTable(db, 'custom_skills'),
      builtin_mcp_servers: await countTable(db, 'builtin_mcp_servers'),
      marketplace_mcp_servers: await countTable(db, 'marketplace_mcp_servers'),
      user_mcp_servers: await countTable(db, 'user_mcp_servers'),
      custom_mcp_servers: await countTable(db, 'custom_mcp_servers'),
      external_markets: await countTable(db, 'external_markets')
    };
    ...
    console.error('[Migration V1] ❌ 迁移失败:', error.message);
...
module.exports = { runMigration, MIGRATION_V1 };