...
async function start() {
  ...
  // ==================== ASR 语音识别 ====================
  try {
    const { initASRService } = require('./services/asr-service');
    initASRService(config);
    console.log('[Server] ASR 服务初始化完成');
  } catch (e: any) {
    console.log('[Server] ASR 服务初始化失败:', e.message);
  }
...
// 邮箱通道路由
  ...
  await transportManager.subscribe(config.channels.messages);
  
  transportManager.onMessage(async (message) => {
    // 判断消息来源频道
    const isReply = message._channel === config.channels.replies;
    ...
    if (isReply) {
      ...
      await transportManager.send(forwardMessage, config.channels.messages);
    } else {
      // 普通消息频道的处理
  ...
  // 订阅回复频道
  await transportManager.subscribe(config.channels.replies);
