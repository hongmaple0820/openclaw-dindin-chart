...
class MCPService {
   ...
   * 获取内置 MCP 列表
   */
  async getBuiltinMCPServers(filters = {}) {
   ...
   * 提交 MCP 到云市场
   */
  async submitToMarketplace(userId, mcpData) {
    const id = uuidv4();
    const now = Date.now();
   ...
   * 获取用户自建 MCP
   */
  async getCustomMCPServers(userId) {
    const rows = await this.db.all(
   ...
   * 创建自建 MCP
   */
  async createCustomMCP(userId, mcpData) {
    const id = uuidv4();
    const now = Date.now();
   ...
   * 删除自建 MCP
   */
  async deleteCustomMCP(id, userId) {
   ...
   * 批准 MCP
   */
  async approveMCP(mcpId, reviewerId, note = '') {
    const now = Date.now();
   ...
   * 拒绝 MCP
   */
  async rejectMCP(mcpId, reviewerId, note = '') {
    const now = Date.now();
