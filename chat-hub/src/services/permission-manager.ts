...
class PermissionManager {
   ...
   * 获取所有权限
   */
  listPermissions(options: { category?: string } = {}): Permission[] {
    this.init();
    if (!this.db) return [];
    ...
    return this.db.prepare(sql).all(...params) as Permission[];
  }
  ...
  getPermission(id: string): Permission | undefined {
    ...
    return this.db.prepare('SELECT * FROM permissions WHERE id = ?').get(id) as Permission | undefined;
  }
   ...
   * 获取所有角色
   */
  listRoles(options: { includePermissions?: boolean } = {}): Role[] {
    this.init();
    if (!this.db) return [];

    const { includePermissions = true } = options;

    const roles = this.db.prepare('SELECT * FROM roles ORDER BY name').all() as Role[];
   ...
   * 创建角色
   */
  createRole(data: CreateRoleData): Role {
    this.init();
    if (!this.db) throw new Error('Database not initialized');
   ...
   * 更新角色
   */
  updateRole(id: string, data: UpdateRoleData): Role {
    this.init();
    if (!this.db) throw new Error('Database not initialized');
   ...
   * 获取 Agent 的角色
   */
  getAgentRoles(agentId: string): Role[] {
    this.init();
    if (!this.db) return [];
   ...
   * 为 Agent 分配角色
   */
  assignRoleToAgent(agentId: string, roleId: string, assignedBy: string | null = null): Role[] {
    this.init();
    if (!this.db) throw new Error('Database not initialized');
   ...
   * 移除 Agent 的角色
   */
  removeRoleFromAgent(agentId: string, roleId: string): boolean {
    this.init();
    if (!this.db) throw new Error('Database not initialized');
   ...
   * 设置 Agent 的角色（替换所有）
   */
  setAgentRoles(agentId: string, roleIds: string[], assignedBy: string | null = null): Role[] {
    this.init();
    if (!this.db) throw new Error('Database not initialized');
   ...
   * 检查 Agent 是否有指定权限
   */
  hasPermission(agentId: string, permissionId: string): boolean {
    this.init();
    ...
    `).get(agentId, permissionId) as { count: number };
   ...
   * 检查 Agent 是否有任一权限
   */
  hasAnyPermission(agentId: string, permissionIds: string[]): boolean {
    this.init();
    if (!this.db) return false;
    ...
    `).get(agentId, ...permissionIds) as { count: number };
   ...
   * 检查 Agent 是否有所有权限
   */
  hasAllPermissions(agentId: string, permissionIds: string[]): boolean {
    this.init();
   ...
   * 获取有指定权限的所有 Agent
   */
  getAgentsWithPermission(permissionId: string): string[] {
    this.init();
    if (!this.db) return [];
    ...
    `).all(permissionId) as { agent_id: string }[]).map(r => r.agent_id);
  }
  ...
  getAgentsWithRole(roleId: string): string[] {
    ...
    `).all(roleId) as { agent_id: string }[]).map(r => r.agent_id);
  }
  ...
  getStats(): Stats {
    this.init();
    if (!this.db) return { permissions: 0, roles: 0, agentRoleAssignments: 0, agentsWithRoles: 0 };

    const permissionCount = (this.db.prepare('SELECT COUNT(*) as count FROM permissions').get() as { count: number }).count;
    const roleCount = (this.db.prepare('SELECT COUNT(*) as count FROM roles').get() as { count: number }).count;
    const agentRoleCount = (this.db.prepare('SELECT COUNT(*) as count FROM agent_roles').get() as { count: number }).count;
...
export type { Permission, Role, CreatePermissionData, CreateRoleData, UpdateRoleData, Stats };